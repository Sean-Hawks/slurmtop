...
Each module is drawn two characters wide and one row tall, which comes out
square on a normal terminal cell. Half-block rendering would be half the width
but makes modules square only if the cell is exactly 1:2, and in practice
scanners choked on it. Squareness costs space: the panel needs 76 columns, so
it only appears when at least 80 columns are left for the queue. `--no-qr`
hides it, `--qr` prints just the code.
